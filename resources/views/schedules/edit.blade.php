        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('schedules.update', $schedule) }}" x-data="{
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            ...
                            <div x-data="{
                                ...
                                <x-text-input id="start_time" class="block mt-1 w-full" type="text" name="start_time"
                                    ...
                                    )" placeholder="0900" maxlength="5" required
                        ...
                        <div x-show="isWebMeeting" x-transition.duration.300ms
                            ...
                            <div>
                                ...
                                <select id="meeting_type" name="meeting_type"
                                    ...
                                    {{-- selected 属性で初期値を選択 --}}
                                    <option value="Zoom"
                                        {{ old('meeting_type', $schedule->meeting_type) == 'Zoom' ? 'selected' : '' }}>
                                        Zoom</option>
                                    <option value="Google Meet"
                                        {{ old('meeting_type', $schedule->meeting_type) == 'Google Meet' ? 'selected' : '' }}>
                                        Google Meet</option>
                                    <option value="その他"
                                        {{ old('meeting_type', $schedule->meeting_type) == 'その他' ? 'selected' : '' }}>
                                        その他</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="meeting_url" class="block mt-1 w-full" type="url"
                                    name="meeting_url" :value="old('meeting_url', $schedule->meeting_url)" />
                            ...
                            <div>
                                ...
                                <textarea id="participants_memo" name="participants_memo"
                                    class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                    rows="2">{{ old('participants_memo', $schedule->participants_memo) }}</textarea>
                                ...
                                <select id="vehicle_id" name="vehicle_id"
                                    ...
                                    @foreach ($vehicles as $vehicle)
                                        <option value="{{ $vehicle->id }}" {{-- createの場合 --}}
                                            {{ old('vehicle_id', optional($schedule->vehicleReservation)->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
                                            {{ $vehicle->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="flex items-center justify-end mt-6">
                            ...
                            <button type="button"
                                ...
                                class="text-sm text-red-600 hover:text-red-900 underline mr-auto">
                                削除する
                            </button>
                    ...
                    {{-- 削除用フォーム --}}
                    <form id="delete-form" action="{{ route('schedules.destroy', $schedule) }}" method="POST"
                        class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
