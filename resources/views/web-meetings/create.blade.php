<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            {{-- 一覧に戻る際、日付を保持 --}}
            <a href="{{ route('web-meetings.index', ['date' => $defaultDate]) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                一覧に戻る
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('web-meetings.store') }}">
                        @csrf

                        <div class="mb-4">
                            <label for="meeting_date" class="block text-sm font-medium text-gray-700">日付 <span class="text-red-500">*</span></label>
                            <input type="date" name="meeting_date" id="meeting_date" value="{{ old('meeting_date', $defaultDate) }}" required
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex space-x-4">
                                <label class="inline-flex items-center">
                                    <input type="radio" name="meeting_type" value="zoom" class="form-radio text-indigo-600" {{ old('meeting_type') == 'zoom' ? 'checked' : '' }} required>
                                    <span class="ml-2">Zoom</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input type="radio" name="meeting_type" value="google_meet" class="form-radio text-indigo-600" {{ old('meeting_type') == 'google_meet' ? 'checked' : '' }}>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="participants_memo" id="participants_memo" rows="3"
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('participants_memo') }}</textarea>
